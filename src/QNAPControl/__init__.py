...
class QnapLCD:
    def __init__(self, handler:Callable[[Response, int|Key|bytes|None], None], port:str='/dev/ttyS1', speed:int=1200, rows:int=2, columns:int=16):
        self.serial: Serial = Serial(port, speed)
        self.columns:int = columns
        self.rows:int = rows

        self.handler:Callable[[Response, int|Key|bytes|None], None] = handler
        self.reader: Thread = Thread(target=self.handle_response)
        self.reader.start()
            ...
            if self.serial.read()[0] in PREAMBLES:
                match self.serial.read():
                    ...
                    case Response.PROTOCOL.value:
                        self.handler(Response.PROTOCOL, int.from_bytes(self.serial.read(2)))
    ...
    def backlight(self, on: bool = True):
        _ = self.serial.write(bytes([Commands.BASE.value, Commands.BACKLIGHT.value, 0x01 if on else 0x00]))

    def write(self, msg: str, row: int = 1):
            msg = msg[:self.columns]
            _ = self.serial.write(bytes([Commands.BASE.value, Commands.DISPLAY_CHAR.value, (row - 1) % self.rows, len(msg)]))
